        ...
        try:
            ...
            for step in steps or []:
                ...
                try:
                    ...
                    self.logger.error("Not found data")
                    return []
                if span.count() != 1:
                    self.logger.error("Not found data")
                    return []
            ...
            for i in range(items.count()):
                ...
                if save_to:
                    save_path = os.path.join(os.path.abspath(save_to), download.suggested_filename)
                else:
            ...
            if isinstance(e, Error) and e.message.startswith(
                "Page.evaluate: NotAllowedError: Failed to execute 'readText' on 'Clipboard': Document is not focused."
            ):
                return self.download_files(url, file, steps, save_to)
            raise e
