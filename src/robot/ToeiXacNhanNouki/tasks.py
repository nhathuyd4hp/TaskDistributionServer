...
def process_schedu_email_content(content: str) -> list[str]:
    ...
    def extract_data_building(building: str):
        ...
        return {"construction": department, "quantity": project_count, "details": project_ids}
...
def run(
    ...
    for ID in mailbox["ID"].to_list():
        ...
        constructions: list[dict] = [
            ...
            if any(keyword in item.get("construction", "") for keyword in PROCESS_CONSTRUCTIONS)
        ]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            for construction in constructions:
                for construction_id in construction.get("details"):
                    ...
                    future_information = executor.submit(
                        web_access.get_information, construction_id=construction_id, fields=FIELDS
                    )
                    job_timeline = future_timeline.result()
                    web_access_information = future_information.result()
                    web_access_information = web_access_information.sort_values(
                        by="案件番号", ascending=True
                    ).reset_index(drop=True)
                    print(f"web_access_information: {web_access_information}")
                    if job_timeline is None:
                        data.append(
                            [None, None, construction_id, None, None, None, False, "KHÔNG LẤY ĐƯỢC THÔNG TIN Ở TOEUI"]
                        )
                        continue
                    if web_access_information.empty:
                        data.append(
                            [
                                ...
                                None,
                                False,
                                "KHÔNG LẤY ĐƯỢC THÔNG TIN Ở WEB ACCESS",
                            ]
                        )
                        continue
                    ...
                    for index, 案件番号 in enumerate(web_access_information["案件番号"].to_list()):
                        ...
                        data.append(
                            [
                                ...
                                row["物件名"],
                                construction_id,
                                touei_endtime.strftime("%Y-%m-%d") if touei_endtime is not None else touei_endtime,
                                (
                                    web_access_endtime.strftime("%Y-%m-%d")
                                    if web_access_endtime is not None
                                    else web_access_endtime
                                ),
                                (
                                    (web_access_endtime - touei_endtime)
                                    if web_access_endtime is not None and touei_endtime is not None
                                    else None
                                ),
                                result_一括操作[0],
                                result_一括操作[1],
