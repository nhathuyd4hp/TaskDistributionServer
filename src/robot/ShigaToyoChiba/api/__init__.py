...
class APISharePoint:
    ...
    def get_metadata(self, site_id: str, breadcrumb: str) -> dict | None:
        """
    ...
    def download_item(self, site_id: str, breadcrumb: str, save_to: str) -> bool:
        try:
            ...
            item_metadata = requests.get(
                url=f"https://graph.microsoft.com/v1.0/sites/{site_id}/drive/root:/{breadcrumb}", headers=headers
            ).json()
    ...
    def rename_breadcrumd(self, siteId: str, driveId: str, itemId: str, new_name: str) -> bool:
        response = requests.patch(
            url=f"https://graph.microsoft.com/v1.0/sites/{siteId}/drives/{driveId}/items/{itemId}",
    ...
    def write(
        ...
        data = {"values": data}
        url = f"https://graph.microsoft.com/v1.0/sites/{siteId}/drives/{driveId}/items/{itemId}/workbook/worksheets('{sheet}')/range(address='{range}')"
        response = requests.patch(
            url=url,
            json=data,
            headers={"Authorization": f"Bearer {self._get_access_token()}", "Content-Type": "application/json"},
